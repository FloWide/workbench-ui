...
export namespace Playback {
    ...
    type PlayBackData = PlayBackElement[];
    interface LeafletArgs {
        playback: EventPlayerBase;
    }
    interface PlayBackEvent<EventArgs = any> {
        name: string;
        do(leafletArgs: LeafletArgs, args: EventArgs): void;
        undo(leafletArgs: LeafletArgs, args: EventArgs): void;
    }

    type TrackEventHandler = (track: Polyline) => void;
    class TrackableMarker extends Marker {
        private tracks;
        private initalTrackColor;
        private latestTrack;
        private previousDoTrack;
        private previousColor;
        private trackLayers;
        constructor(latlng: LatLngExpression, options?: MarkerOptions, trackColor?: string);
        set TracksLayer(g: LayerGroup);
        trackForward(latlng: LatLngExpression, doTrack?: boolean, trackColor?: string): void;
        trackBackward(doTrack?: boolean): void;
        removeTracks(): void;
    }
    export class EventPlayerBase extends LayerGroup {
        protected markers: Record<string, TrackableMarker>;
        protected events: Record<string, PlayBackEvent>;
        protected genericLayers: Record<string, Layer>;
        constructor(options?: LayerOptions);
        createMarker(marker: TrackableMarker, name: string): TrackableMarker;
        createGenericLayer<T extends Layer>(layer: T, id: string): T;
        removeGenericLayer(id: string): void;
        removeMarker(id: string): void;
        registerEvents(event: PlayBackEvent | PlayBackEvent[]): void;
        getMarkerById(id: string): TrackableMarker | undefined;
        getLayerById<T extends Layer>(id: string): T | undefined;
        getMap(): L.Map | undefined;
    }
    export class PlayBack extends EventPlayerBase {
        private tracks;
        private data;
        private timeMs;
        private tracking;
        private forward;
        constructor(data: PlayBackData, options?: LayerOptions);
        createMarker(marker: TrackableMarker, name: string): TrackableMarker;
        removeMarker(id: string): void;
        set Time(ms: number);
        get Time(): number;
        get MinimumTime(): number;
        get MaximumTime(): number;
        get Track(): boolean;
        set Track(t: boolean);
        get Data(): PlayBackData;
        private execEvents;
        private findClosestEventsToTime;
        private findClosestEventIndex;
    }
    export class LivePlay extends EventPlayerBase {
        private webSocket;
        constructor(webSocket?: any | string, options?: LayerOptions);
        setWebsocket(ws: any | string): void;
        onEvent(ev: PlayBackElement): void;
        private onWsMessage;
    }

    export class PlayControl extends Control {
        options: ControlOptions;
        private timer;
        private playing;
        private timeLabel;
        private forwardButton;
        private backwardsButton;
        private playback;
        private timeline;
        private map;
        private spinner;
...
export namespace CustomTransformation{
    type Array3x3 = [
        [
            number,
            number,
    ...
    interface MatrixTransformationConfig {
        transform: Array3x3;
        untransform: Array3x3;
    }
    export class MatrixTransformation extends Transformation {
        private transformMatrix;
        private inverseMatrix;
        constructor(config?: MatrixTransformationConfig);
        _transform(point: Point, scale?: number): Point;
        transform(point: Point, scale?: number): Point;
        untransform(point: Point, scale?: number): Point;
        get transformationMatrix(): any;
        get inverseTransformationMatrix(): any;
        private fromArray3x3;
    }

    export class CustomCRS {
        projection: any;
        transformation: Transformation;
        constructor(transformationConfig?: MatrixTransformationConfig);
        set Transformation(transformationConfig: MatrixTransformationConfig);
    }

    export class ImageOverlayExcludeCRS extends ImageOverlay {
        constructor(imageUrl: string, bounds: LatLngBounds, options?: ImageOverlayOptions);
        onAdd(map: L.Map): this;
        setBounds(bounds: LatLngBounds): this;
        private _setBounds;
        private _toLatLngBounds;
    }

    export class CoordinateDisplay extends Control {
        private displayOn;
        options: ControlOptions;
        private map;
        private xPositionLabel;
        private yPositionLabel;
        constructor(displayOn?: 'mousemove' | 'click', options?: ControlOptions);
        onAdd(map: L.Map): HTMLElement;
    ...
    export class Grid extends LayerGroup {
        private guideLines;
        private snapPoints;
        constructor(guideLines?: boolean, snapPoints?: boolean);
        onAdd(map: L.Map): this;
        redraw(): void;
        get ShowGuideLines(): boolean;
        set ShowGuideLines(value: boolean);
        get SnapPoints(): boolean;
        set SnapPoints(value: boolean);
        private redrawLines;
